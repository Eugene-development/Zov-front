	...
	{#if step === 0}
		<div class="py-2 text-center">
			<p class="mx-auto mb-6 px-12 text-sm leading-relaxed text-secondary">
				Ответьте на несколько простых вопросов о вашей будущей мебели, и мы рассчитаем её примерную
				стоимость
			</p>
		...
		<div class="mb-4">
			...
			<div class="flex flex-col gap-3">
				{#each ['Кухня', 'Шкаф', 'Гардероб'] as option (option)}
					<button
						class="w-full border border-border-light p-4 text-left text-sm transition-colors duration-300 hover:border-secondary {furnitureType ===
						option
							? 'border-secondary bg-secondary/5'
							: ''}"
						onclick={() => selectFurniture(option)}>{option}</button
					>
				{/each}
			</div>
		</div>
		...
		<div class="mb-4">
			...
			<div class="flex flex-col gap-3">
				{#each q2Options as option (option)}
					<button
						class="w-full border border-border-light p-4 text-left text-sm transition-colors duration-300 hover:border-secondary {subType ===
						option
							? 'border-secondary bg-secondary/5'
							: ''}"
						onclick={() => selectSubType(option)}>{option}</button
					>
				{/each}
			</div>
			<button
				...
				class="mt-4 text-xs text-muted underline-offset-2 hover:underline">← Назад</button
			>
		</div>
		...
		<div class="mb-4">
			...
			<div class="flex flex-col gap-3">
				{#each facadeOptions as option (option)}
					<label
						class="flex cursor-pointer items-center gap-3 border border-border-light p-4 text-sm transition-colors duration-300 hover:border-secondary {facades.includes(
							option
						)
							? 'border-secondary bg-secondary/5'
							: ''}"
			...
			<div class="mt-6 flex items-center justify-between">
				<button
					...
					class="text-xs text-muted underline-offset-2 hover:underline">← Назад</button
				>
				<button
					type="button"
	...
	{:else if step === 4}
		<div class="mb-4">
			...
			<textarea
				...
				rows="4"
				placeholder="Например: длина кухни 3 м, мебель в потолок 2.7 м..."
				class="w-full resize-none border border-border-light bg-transparent p-4 text-sm text-primary transition-colors duration-300 outline-none focus:border-secondary"
			></textarea>
			<div class="mt-6 flex items-center justify-between">
				<button
					type="button"
					onclick={() => {
						step = skipFacades ? 2 : 3;
					}}
					class="text-xs text-muted underline-offset-2 hover:underline">← Назад</button
				>
				<button
